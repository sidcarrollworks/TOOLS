...
import { ShaderExport } from "./ShaderExport";
...
export class ShaderAppFacade extends EventEmitter implements IShaderAppFacade {
  ...
  /**
   * ShaderExport instance for handling export operations
   * @private
   */
  private exporter: ShaderExport | null = null;
  ...
  constructor(app?: ShaderApp, config?: Partial<FacadeConfig>) {
    ...
    // If app is provided, initialize the exporter
    if (app) {
      this.exporter = new ShaderExport(app, this.config);
      this.setupExporterEventForwarding();
    }
  ...
  /**
   * Set up event forwarding from exporter to facade
   * @private
   */
  private setupExporterEventForwarding(): void {
    if (!this.exporter) return;

    // Forward export-related events from the exporter to facade listeners
    this.exporter.on("export-started", (data) =>
      this.emit("export-started", data)
    );
    this.exporter.on("export-complete", (data) =>
      this.emit("export-complete", data)
    );
    this.exporter.on("error", (data) => this.emit("error", data));
  }
  ...
  public async initialize(container: HTMLElement): Promise<void> {
    ...
    try {
      ...
      // Initialize the exporter if not already done
      if (!this.exporter && this.app) {
        this.exporter = new ShaderExport(this.app, this.config);
        this.setupExporterEventForwarding();
      }
  ...
  public async exportAsImage(
    ...
    // Delegate to the ShaderExport instance
    return this.exporter!.exportAsImage(options);
  }
  ...
  public async exportAsCode(options: ExportCodeOptions = {}): Promise<string> {
    ...
    // Delegate to the ShaderExport instance
    return this.exporter!.exportAsCode(options);
  }
  ...
  public validateExport(): Record<string, any> {
    ...
    // Delegate to the ShaderExport instance
    return this.exporter!.validateExport();
  }
