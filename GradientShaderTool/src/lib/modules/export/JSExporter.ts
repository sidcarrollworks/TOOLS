...
export class JSExporter {
  ...
  async generateJavaScriptOnly(): Promise<string> {
    ...
    // Get shader code
    const shaderCode =
      await this.app.exportManager.shaderExporter.generateShaderCode();
...
function createGradientShader(container) {
  ...
  // Create material with wireframe property
  material.wireframe = ${params.showWireframe};

  // Create geometry and mesh based on the selected type
  ${
    params.geometryType === "sphere"
      ? `const geometry = new THREE.SphereGeometry(${params.sphereRadius}, ${params.sphereWidthSegments}, ${params.sphereHeightSegments});`
